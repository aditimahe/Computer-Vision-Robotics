...
class CNN(torch.nn.Module):
    def __init__(self, num_bins): 
        ...
        self.conv1 = torch.nn.Conv2d(3, 16, kernel_size=(3,3)) # 3 input channels, 16 output channels
        self.conv1_bn = torch.nn.BatchNorm2d(16) # Batch normalization after 1st conv layer

        self.pool1 = torch.nn.MaxPool2d((2, 2)) # Max pooling
        self.relu = torch.nn.ReLU() #Relu

        self.conv2 = torch.nn.Conv2d(16, 16, kernel_size=(5,5))
        self.conv2_bn = torch.nn.BatchNorm2d(16) # Batch normalization after 2nd conv layer

        self.conv3 = torch.nn.Conv2d(16, 16, kernel_size=(7,7))
        self.conv3_bn = torch.nn.BatchNorm2d(16) # Batch normalization after 3rd conv layer

        # Fully connected layers
        self.fc1 = torch.nn.Linear(1472, 512)
        self.fc2 = torch.nn.Linear(512, 128)
        self.fc3 = torch.nn.Linear(128, 64)
        self.fc4 = torch.nn.Linear(64, num_bins)
    ...
    def forward(self, x):
        ...
        # Pass input through conv layer, then batch normalization, then relu, then pooling
        x = self.pool1(self.relu(self.conv1_bn(self.conv1(x))))
        x = self.pool1(self.relu(self.conv2_bn(self.conv2(x))))
        x = self.pool1(self.relu(self.conv3_bn(self.conv3(x))))
...
class dataloader(torch.utils.data.Dataset):
    ...
    def normalize_to_zero_mean(self):
        #---FILL ME IN-----
        # Mean of training dataset is 94.98759942130943
        # Subtract from the training data and validation data
        self.images = np.subtract(self.images, 94.98759942130943)
        #------------------
        pass
